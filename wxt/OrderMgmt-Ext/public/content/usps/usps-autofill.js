...
function fillPackageInformationDirectly(orderData) {
    ...
    // Weight: by name
    const weightName = USPS_FORM_FIELDS.weightLbs;
    const weightField = getFormInputByName(weightName);
    if (weightField) {
        console.log('✅ Found weight field with name:', weightName);
    }
    ...
    if (weightField && allocationPackage.weight) {
        ...
        console.log('🔍 Expected name:', weightName);
    }
    
    // Length / width / height: by name (USPS_FORM_FIELDS), same as weight — Veeqo "depth" maps to length input
    const lengthName = USPS_FORM_FIELDS.length;
    const widthName = USPS_FORM_FIELDS.width;
    const heightName = USPS_FORM_FIELDS.height;
    const lengthField = getFormInputByName(lengthName);
    const widthField = getFormInputByName(widthName);
    const heightField = getFormInputByName(heightName);
    if (lengthField) console.log('✅ Found length field with name:', lengthName);
    if (widthField) console.log('✅ Found width field with name:', widthName);
    if (heightField) console.log('✅ Found height field with name:', heightName);

    const lengthData = allocationPackage.depth != null && allocationPackage.depth !== ''
        ? allocationPackage.depth
        : allocationPackage.length;
    if (lengthField && lengthData != null && lengthData !== '') {
        const value = parseFloat(lengthData) || 0;
        lengthField.value = value.toString();
        triggerInputEvent(lengthField);
        console.log('✅ Filled length (depth or length):', value);
    } else {
        console.log('❌ length field not found or no depth/length data');
        console.log('🔍 Expected name:', lengthName);
    }

    if (widthField && allocationPackage.width != null && allocationPackage.width !== '') {
        const value = parseFloat(allocationPackage.width) || 0;
        widthField.value = value.toString();
        triggerInputEvent(widthField);
        console.log('✅ Filled width:', value);
    } else {
        console.log('❌ width field not found or no width data');
        console.log('🔍 Expected name:', widthName);
    }

    if (heightField && allocationPackage.height != null && allocationPackage.height !== '') {
        const value = parseFloat(allocationPackage.height) || 0;
        heightField.value = value.toString();
        triggerInputEvent(heightField);
        console.log('✅ Filled height:', value);
    } else {
        console.log('❌ height field not found or no height data');
        console.log('🔍 Expected name:', heightName);
    }
}
...
function fillPackageInformation(orderData) {
    ...
    // Fill Weight (lbs) — by name
    const weightName = USPS_FORM_FIELDS.weightLbs;
    const weightField = getFormInputByName(weightName);
    console.log('🔍 Weight field found:', !!weightField, 'name:', weightName);
    console.log('🔍 Weight data available:', !!allocationPackage.weight, 'value:', allocationPackage.weight);
    ...
    if (weightField && allocationPackage.weight) {
        ...
        console.log('❌ Weight field not found or no weight data');
        if (!weightField) console.log('❌ Expected name:', weightName);
        if (!allocationPackage.weight) console.log('❌ No weight in allocation_package');
    }

    // Fill Length (Veeqo depth or length) — by name
    const lengthName = USPS_FORM_FIELDS.length;
    const lengthField = getFormInputByName(lengthName);
    const lengthData = allocationPackage.depth != null && allocationPackage.depth !== ''
        ? allocationPackage.depth
    ...
    if (lengthField && lengthData != null && lengthData !== '') {
        ...
        lengthField.value = value.toString();
        triggerInputEvent(lengthField);
        console.log('✅ Filled length (depth/length):', value);
    } else {
        console.log('Length field not found or no depth/length data');
        console.log('🔍 Expected name:', lengthName);
    }
...
async function fillFormWithOrderData(orderData) {
    ...
    // Also clear package fields (L/W/H/weight by name; dropdown by id)
    [USPS_FORM_FIELDS.weightLbs, USPS_FORM_FIELDS.length, USPS_FORM_FIELDS.width, USPS_FORM_FIELDS.height].forEach(
        (name) => {
            const field = getFormInputByName(name);
            if (field) {
                field.value = '';
                triggerInputEvent(field);
            }
    ...
    const packageTypeEl = document.getElementById(USPS_FORM_FIELDS.packageTypeDropdown);
    if (packageTypeEl && packageTypeEl.tagName === 'SELECT') {
        packageTypeEl.selectedIndex = 0;
        triggerInputEvent(packageTypeEl);
    }
    ...
    if (allocationPackage) {
        ...
        // Fill weight + L/W/H by name (USPS_FORM_FIELDS)
        const weightField = getFormInputByName(USPS_FORM_FIELDS.weightLbs);
        if (weightField && allocationPackage.weight) {
            const weightOz = parseFloat(allocationPackage.weight) || 0;
            const wLbs = Math.floor(weightOz / 16);
            weightField.value = wLbs.toString();
            triggerInputEvent(weightField);
            console.log('✅ Filled Weight:', wLbs, 'lbs');
        }
        const lengthData = allocationPackage.depth != null && allocationPackage.depth !== ''
            ? allocationPackage.depth
            : allocationPackage.length;
        const lengthField = getFormInputByName(USPS_FORM_FIELDS.length);
        if (lengthField && lengthData != null && lengthData !== '') {
            const value = parseFloat(lengthData) || 0;
            lengthField.value = value.toString();
            triggerInputEvent(lengthField);
        ...
        const widthField = getFormInputByName(USPS_FORM_FIELDS.width);
        if (widthField && allocationPackage.width != null && allocationPackage.width !== '') {
            const value = parseFloat(allocationPackage.width) || 0;
            widthField.value = value.toString();
            triggerInputEvent(widthField);
        ...
        const heightField = getFormInputByName(USPS_FORM_FIELDS.height);
        if (heightField && allocationPackage.height != null && allocationPackage.height !== '') {
            const value = parseFloat(allocationPackage.height) || 0;
            heightField.value = value.toString();
            triggerInputEvent(heightField);
